...
class StudentsController extends \BaseController {
	...
	public function export(){
		...
		$data = array(1 => array ('Roll', 'Full Name','User Name','E-mail','Gender','Address','Phone No','Mobile No','Class','password','activated','birthday'));
		$student = User::where('role','student')->get();
		foreach ($student as $value) {
			$data[] = array ($value->studentRollId, $value->fullName,$value->username,$value->email,$value->gender,$value->address,$value->phoneNo,$value->mobileNo,isset($classArray[$value->studentClass]) ? $classArray[$value->studentClass] : "","",$value->activated,$value->birthday);
		}
	...
	function attendance($id){ 
		return attendance::where('studentId',$id)->get();
	}
