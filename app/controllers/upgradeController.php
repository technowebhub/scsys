...
class upgradeController extends \BaseController {
		...
		try{
			...
			if(Schema::hasTable('settings')){
				...
				if($testInstalled->count() > 0){
					...
					if($testInstalled->fieldValue >= 1.3){
	...
	public function proceed()
	{
		...
		if(Input::get('nextStep') == "2"){
			...
			$testInstalled = settings::where('fieldName','thisVersion')->get();
			if($testInstalled->count() == 0){
				//Upgrade from first version to 1.2
				DB::unprepared(file_get_contents('app/storage/dbsqlUp'));
				$settings = new settings();
				$settings->fieldName = 'thisVersion';
				$settings->fieldValue = '1.3';
				$settings->save();
			}
		}
